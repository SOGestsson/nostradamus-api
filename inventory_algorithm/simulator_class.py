...
class inventory_simulator:
    ...
    def __init__(self, invsimdataset):
    ...
    def update_inventory(self, curr_inv, day_nr, outgoing, incoming, initial_shelf_life, backorder):
        ...
        while outgoing_remaining > 0 and len(
                curr_inv) > 1:  # skip the last element, that is used for negative inventory
    ...
    def calc_inv_at_next_delivery(self, current_inventory, forecast, deliveries, other_deliveries, index_from,
                                  index_this_delivery, index_next_delivery, backorder):
        currentinv = copy.deepcopy(current_inventory)

        # start with the current deliveries at the end of the current day
        currentinv, _, _, _ = self.update_inventory(currentinv, index_from, 0,
                                               deliveries[index_from] + other_deliveries[index_from],
                                               index_next_delivery - index_from + 1, backorder)
        ...
        for i in range(index_from + 1, index_this_delivery + 1):
            currentinv, inv_at_this_deliv, _, _ = self.update_inventory(currentinv, i, forecast[i],
                                                                   deliveries[i] + other_deliveries[i],
                                                                   index_next_delivery - index_from + 1, backorder)
        ...
        for i in range(index_this_delivery + 1, index_next_delivery + 1):
            currentinv, current_total, _, _ = self.update_inventory(currentinv, i, forecast[i],
                                                               deliveries[i] + other_deliveries[i],
                                                               index_next_delivery - index_from + 1, True)

            # return the inventory position at next delivery
        return current_total, inv_at_this_deliv

    def calc_inv_at_next_delivery_with_bypass(self, current_inventory, totalforecast, deliveries, other_deliveries,
                                              index_from, index_this_delivery, index_to):
        # calculate the current total inventory
        currentinv = copy.deepcopy(current_inventory)
        _, current_total, _, _ = self.update_inventory(currentinv, index_from, 0, 0, index_to - index_from + 1, False)
    ...
    def inventory_simulator(self, initial_inv, leadtime, safetystock, forecast, sales, purchases, other_deliveries,
                            backorder, leadtimeforecast, buyfreqforecast, bypass_forecast, min_order_qty, package_size,
                            initial_shelf_life, use_minmax, minmax_min, minmax_max):
        T = len(purchases)
        ...
        for t in range(T - leadtime):
            ...
            current_inventory, curr_inv_pos, lost_sales[t], expired[t] = self.update_inventory(current_inventory, t,
                                                                                          salesandforecast[t], 0,
                                                                                          initial_shelf_life, backorder)

            # check if this is an order day and if the remaining time is long enough to calculate the purchase quantity
            if (purchase_index + 1 < len(purchase_dates)) and (t == purchase_dates[purchase_index]) and (
                    purchase_dates[purchase_index + 1] + leadtime + 1 < T):
                # calculate the inventory position when the next delivery after this will arrive
                if bypass_forecast:
                    inv_pos_at_next_delivery, inv_pos_at_this_delivery = self.calc_inv_at_next_delivery_with_bypass(
                        current_inventory, leadtimeforecast + buyfreqforecast, deliveries, other_deliveries, t,
                        t + leadtime, purchase_dates[purchase_index + 1] + leadtime)
                else:
                    inv_pos_at_next_delivery, inv_pos_at_this_delivery = self.calc_inv_at_next_delivery(current_inventory,
                                                                                                   forecast, deliveries,
                                                                                                   other_deliveries, t,
                                                                                                   t + leadtime,
                                                                                                   purchase_dates[
                                                                                                       purchase_index + 1] + leadtime,
                                                                                                   backorder)
            ...
            current_inventory, inv[t], _, _ = self.update_inventory(current_inventory, t, 0,
                                                               deliveries[t] + other_deliveries[t], initial_shelf_life,
                                                               backorder)
        ...
        for t in range(T - leadtime, T):
            current_inventory, inv[t], lost_sales[t], expired[t] = self.update_inventory(current_inventory, t,
                                                                                    salesandforecast[t],
                                                                                    deliveries[t] + other_deliveries[t],
                                                                                    initial_shelf_life, backorder)

        return inv, purchase_qty, deliveries, lost_sales, expired

    def get_key_value_or_default(self, the_extra_params, the_key, the_defaultvalue, func=lambda x: x):
    ...
    def run_inventory_simulator(self, invsimdataset):
        ...
        while (end_pos < len(dataset_itemid)):
            ...
            if dataset_actual_sale:
                ...
                while (end_of_sale > start_pos) and (
                        math.isnan(dataset_actual_sale[end_of_sale]) or dataset_actual_sale[end_of_sale] < -1000000):
            ...
            safety_stock = self.get_key_value_or_default(js, 'safety_stock',
                                                    z_value * math.sqrt(lead_time) * math.sqrt(the_variance))
            backorder = self.get_key_value_or_default(js, 'backorder', False)

            # lead_time_forecast and buy_freq_forecast will override the forecast column if the bypass_forecast is set to True
            lead_time_forecast = self.get_key_value_or_default(js, 'lead_forecast', 0)
            buy_freq_forecast = self.get_key_value_or_default(js, 'buy_forecast', 0)
            bypass_forecast = self.get_key_value_or_default(js, 'bypass_forecast',
                                                       False)  # (lead_time_forecast + buy_freq_forecast > 0)

            use_minmax = self.get_key_value_or_default(js, 'use_minmax', False)
            minmax_min = self.get_key_value_or_default(js, 'minmax_min', 0)
            minmax_max = self.get_key_value_or_default(js, 'minmax_max', 0)

            manual_safety_stock = self.get_key_value_or_default(js, 'manual_safety_stock', -1)
            if manual_safety_stock > 0:
                safety_stock = manual_safety_stock

            min_order_qty = self.get_key_value_or_default(js, 'min_order_qty', 0)
            package_size = self.get_key_value_or_default(js, 'package_size', 1)

            initial_shelf_life = self.get_key_value_or_default(js, 'initial_shelf_life', end_pos + 1)
            ...
            initial_inv.append(
                [math.inf, 0])  # initialize the inventory, placeholder at the end for negative inventory/backorders
            ...
            inv_results, purchase_qty_results, deliveries_results, lost_sales_results, expired_results = self.inventory_simulator(
                initial_inv, lead_time, safety_stock, the_forecast, the_sale, the_purchases, the_deliveries, backorder,
                lead_time_forecast, buy_freq_forecast, bypass_forecast, min_order_qty, package_size, initial_shelf_life,
                use_minmax, minmax_min, minmax_max)
        ...
        pandasdata = {'item_id': item_id, 'inv': inv, 'purchase_qty': purchase_qty, 'deliveries': deliveries,
                      'lost_sale': lost_sale, 'expired': expired, 'sim_date': sim_date}
        totalresults = pd.DataFrame(pandasdata,
                                    columns=['item_id', 'inv', 'purchase_qty', 'deliveries', 'lost_sale', 'expired',
                                             'sim_date'])
